"""
Media Agent Stub — simulates a client media planning AI agent on AWS/GCP.

Endpoints:
  POST /invoke   Accept a prompt, return a mock media plan response (auth required)
  GET  /health   Health check (no auth)

Environment variables:
  AGENT_NAME             Value returned in agent_name field.   Default: media-agent
...
AGENT_NAME = os.environ.get("AGENT_NAME", "media-agent")
_PORT = int(os.environ.get("PORT", "8002"))

app = FastAPI(
    title="Media Agent Stub",
    description=(
        "Local stub simulating a client media planning AI agent. "
        "Returns mock media plan responses. Requires Bearer JWT auth."
    ),
    version="1.0.0",
