...
## Workshop Prep: WPP Open Agents → M365 Integration

### 1. Understanding WPP's Likely Current Architecture

Based on the agenda (Python, React, GCP, AWS), WPP's agent stack likely looks like:

```
┌──────────────────────────────────────┐
│  WPP Open / Super Agent Platform     │
│  (React Frontend)                    │
└──────────────┬───────────────────────┘
               │
┌──────────────▼───────────────────────┐
...
**Key questions to ask WPP in the workshop:**
...
WPP keeps agents on AWS and GCP. M365 Copilot calls them as API plugins via OpenAPI specs.
...
│  Declarative Agent                                   │
│  ┌─────────────────────────────────────────┐        │
│  │ Instructions: "You are a WPP creative   │        │
│  │ assistant that can call WPP agents..."   │        │
│  │                                          │        │
│  │ Actions:                                 │        │
│  │  • wpp-open-agent (API Plugin)           │        │
│  │  • wpp-creative-agent (API Plugin)       │        │
...
- **Zero re-platforming** — WPP agents stay on AWS/GCP untouched
- **Copilot orchestrates** — M365 Copilot decides when to invoke WPP agents based on user intent
- **Works across all M365 surfaces** — Teams, Word, PPT, Excel, Outlook, BizChat
- **Fast to prototype** — days, not months

**What WPP needs to provide:**
...
```json
// wpp-agent-plugin.json
{
  "schema_version": "v2.1",
  "name_for_human": "WPP Creative Agent",
  "name_for_model": "wpp_creative_agent",
  "description_for_human": "Access WPP's creative AI agents",
  "description_for_model": "Call this plugin when the user wants creative campaign ideas, brand analysis, media planning, or content generation using WPP's specialized AI agents.",
  "auth": {
    "type": "oauth",
    ...
    "scope": "openid profile email wpp.agents.invoke"
  },
  "api": {
    "type": "openapi",
    "url": "https://wpp-agents.azure-api.net/openapi.json"
  }
}
```
...
│  │  • Routes to appropriate WPP agent            │     │
...
- Can combine **Microsoft Graph data** (user context, documents, calendar) with WPP agent calls
- Can add **Azure AI Search** over WPP's content for RAG
- Streaming support
- Can implement **Super Agent** pattern in Azure that orchestrates multiple WPP sub-agents

**Azure resources needed:**
- Azure App Service or Azure Container Apps (host the gateway)
- Azure Bot Service registration
- Azure AI Search (optional, for RAG over WPP content)
...
Rebuild WPP agents natively in Azure AI Foundry (formerly Azure AI Studio).
...
│  Azure AI Foundry                                     │
│  ┌────────────────────────────────────────────┐      │
│  │  WPP Super Agent (Prompt Flow / Agent Svc)  │      │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐   │      │
...
- WPP wants deepest M365 integration (inline Copilot experiences, Office add-ins)
- WPP wants to use Azure OpenAI models instead of Vertex AI
...
|---|---|---|---|
| **WPP effort** | Low (expose REST APIs + OpenAPI) | Low-Medium (no agent changes) | High (rebuild agents) |
...
| **WPP GCP dependency** | Retained | Retained | Eliminated |

---

### 4. The "Super Agent" Architecture in M365

For WPP's "Super Agent" concept (an orchestrator that routes to specialized sub-agents), there's a clean M365 mapping:

```
M365 Copilot (BizChat / Teams / Word / PPT)
    │
    ▼
Declarative Agent: "WPP Super Agent"
    │
    ├── Action: wpp-creative-agent  (API Plugin → GCP)
    ├── Action: wpp-media-agent     (API Plugin → GCP)  
    ├── Action: wpp-analytics-agent (API Plugin → GCP)
    ├── Knowledge: SharePoint site with WPP brand guidelines
    └── Knowledge: Microsoft Graph (user's files, emails, calendar)
```

M365 Copilot **already is** a super agent — it has built-in reasoning to decide which action/plugin to call based on user intent. WPP doesn't need to rebuild their orchestration; Copilot handles it.

Alternatively, if WPP's Super Agent has **custom routing logic** that Copilot can't replicate, use Pattern B and keep the orchestration in the gateway.
...
- Does WPP want per-user auth (user identity to AWS/GCP) or service-to-service via Okta clients?
...
| **Frontend** | Adaptive Cards (if Pattern B) | React (existing WPP UI) |
...
1. **Start with a demo ask**: "Can we see a WPP agent invoked via REST API today?" — if yes, Pattern A is immediately viable.
...
3. **Auth is the longest pole**: Get clarity on WPP's current auth model early. Everything else is configuration.

4. **Propose a phased approach**:
   - **Phase 1** (weeks): Pattern A — API Plugin. One WPP agent callable from M365 Copilot. Proof of concept.
...
6. **Ask WPP**: "What does the user journey look like today?" Get them to walk through a real user flow end-to-end — this reveals the API contracts you'll need to wrap.
...
| **Azure AI Search** | RAG over WPP content |
...
This should give you a strong technical foundation for the workshop. The key message: **WPP doesn't need to re-platform to get into M365 — API Plugins let their GCP agents be called from Copilot today.** The question is how deep/rich they want the integration to be.
